...
namespace MicrosoftResearch.Infer.Models.User
{
	...
	/// </remarks>
	public class Model6_EP : IGeneratedAlgorithm
	{
		#region Fields
		...
		public void Changed_Cloudy_WetGrass()
		{
			...
			this.Cloudy_marginal = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli27);
			...
			Bernoulli Rain_F = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli30);
			this.Rain_marginal_F = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli30);
			// Message to use of 'Rain'
			Bernoulli Rain_use_F = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli30);
			// Message from use of 'Rain'
			Bernoulli Rain_use_B = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli30);
			if (this.cloudy) {
				// Message to 'Rain' from Copy factor
				Rain_F = Factor.Copy<Bernoulli>(vBernoulli30);
			}
			// The constant 'vBernoulli31'
			Bernoulli vBernoulli31 = Bernoulli.FromLogOdds(-1.3862943611198906);
			if (!this.cloudy) {
				// Message to 'Rain' from Copy factor
				Rain_F = Factor.Copy<Bernoulli>(vBernoulli31);
			}
			...
			if (!this.cloudy) {
				// Message to 'Rain_use' from DerivedVariable factor
				Rain_use_F = Factor.Copy<Bernoulli>(vBernoulli31);
			}
			...
			if (this.cloudy) {
				...
				Sprinkler_F = Factor.Copy<Bernoulli>(vBernoulli28);
			}
			if (!this.cloudy) {
				// Message to 'Sprinkler' from Copy factor
				Sprinkler_F = Factor.Copy<Bernoulli>(vBernoulli27);
			}
			// Sprinkler_F is now updated in all contexts
			// Message to use of 'Sprinkler'
			Bernoulli Sprinkler_use_F = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli28);
			if (this.cloudy) {
			...
			if (!this.cloudy) {
				// Message to 'Sprinkler_use' from DerivedVariable factor
				Sprinkler_use_F = Factor.Copy<Bernoulli>(vBernoulli27);
			}
			...
			Bernoulli Rain_cond_Sprinkler_0_selector_B = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli30);
			...
			for(int _gateind = 0; _gateind<2; _gateind++) {
				Rain_cond_Sprinkler_B[_gateind] = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli30);
			}
			...
			Bernoulli Rain_cond_Sprinkler_1_selector_B = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli30);
			...
			if (!this.cloudy) {
				// Message to 'Rain_marginal' from DerivedVariable factor
				this.Rain_marginal_F = DerivedVariableOp.MarginalAverageConditional<Bernoulli>(Rain_use_B, vBernoulli31, this.Rain_marginal_F);
			}
			...
			if (!this.cloudy) {
				// Message to 'Sprinkler_marginal' from DerivedVariable factor
				this.Sprinkler_marginal_F = DerivedVariableOp.MarginalAverageConditional<Bernoulli>(Sprinkler_selector_B, vBernoulli27, this.Sprinkler_marginal_F);
			}
